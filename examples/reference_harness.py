...
#: RFC 3526 (MODP) and RFC 7919 (FFDHE) safe-prime groups, generator 2.
#: A vector set names its group and never sends the parameters, so every
#: implementation carries them. A real harness would take these from whatever
#: library it already links against; they are spelled out here because this
#: file deliberately imports nothing.
SAFE_PRIME_GROUPS = {
    "MODP-2048": int(
        ""
...
SAFE_PRIME_GENERATOR = 2


def _sp_size(prime: int) -> int:
    return (prime.bit_length() + 7) // 8


def safe_primes_keygen(request: dict[str, Any]) -> dict[str, str]:
    """Generate a key pair in a named safe-prime group."""
    prime = SAFE_PRIME_GROUPS.get(request["safePrimeGroup"])
    if prime is None:
        return {"error": "unsupported"}
    size = _sp_size(prime)
    # x in [1, q-1] where q = (p-1)/2. Generator 2 has order q, so a larger x
    # still yields a valid y -- nothing local can catch the mistake.
    x = secrets.randbelow((prime - 1) // 2 - 1) + 1
    y = pow(SAFE_PRIME_GENERATOR, x, prime)
    return {"x": x.to_bytes(size, "big").hex().upper(), "y": y.to_bytes(size, "big").hex().upper()}


def safe_primes_keyver(request: dict[str, Any]) -> dict[str, bool] | dict[str, str]:
    ...
    if not 1 < y < prime - 1:
        return {"testPassed": False}
    return {"testPassed": pow(SAFE_PRIME_GENERATOR, int(request["x"], 16), prime) == y}


def kas_ffc_keygen(request: dict[str, Any]) -> dict[str, str]:
    """Generate an ephemeral finite-field key pair."""
    prime = SAFE_PRIME_GROUPS.get(request["domainParameterGenerationMode"])
    if prime is None:
        return {"error": "unsupported"}
    size = _sp_size(prime)
    private = secrets.randbelow((prime - 1) // 2 - 1) + 1
    public = pow(SAFE_PRIME_GENERATOR, private, prime)
    return {
        "privateKey": private.to_bytes(size, "big").hex().upper(),
        "publicKey": public.to_bytes(size, "big").hex().upper(),
    }


def kas_ffc_ssc(request: dict[str, Any]) -> dict[str, str]:
    """Compute the finite-field shared secret, left-padded to the modulus."""
    prime = SAFE_PRIME_GROUPS.get(request["domainParameterGenerationMode"])
    if prime is None:
        return {"error": "unsupported"}
    peer = int(request["peerPublic"], 16)
    if not 1 < peer < prime - 1:
        return {"error": "unsupported"}
    z = pow(peer, int(request["privateKey"], 16), prime)

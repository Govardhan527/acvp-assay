...
- Project and task ID: ACVP Runner/Adapter - A11
...
- Commit/link/path: `src/acvp_runner/reporter.py` and `tests/unit/test_reporter.py`.
- Blocker, if any: none.
...
- Project and task ID: ACVP Runner/Adapter - review follow-up (A10/A11)
...
- Commit/link/path: `src/acvp_runner/models.py`, `src/acvp_runner/comparator.py`, `src/acvp_runner/reporter.py`, `tests/unit/test_models.py`, `tests/unit/test_reporter.py`.
- Blocker, if any: none.
- Next unchecked ID: A12 - implement the `run` command, output file, strict mode, and documented exit codes.

## 2026-09-02 - A12 the `run` command

- Project and task ID: ACVP Runner/Adapter - A12
- Done condition: `acvp-runner run VECTOR_FILE [--output RESULT_FILE] [--strict]` parses a vector file and its sibling `expectedResults.json`, executes every case through the OpenSSL-backed provider, writes the JSON report to stdout or a file, and returns a documented exit code (0 hard-pass, 1 case failures or `--strict` soft failures, 2 the run could not start).
...
- Commit/link/path: `src/acvp_runner/{parser,models,runner,cli}.py`, `tests/unit/test_expected_results_parser.py`, `tests/unit/test_runner.py`, `tests/unit/test_cli.py`.
- Blocker, if any: none.
...
- Project and task ID: ACVP Runner/Adapter - A13
...
- Evidence produced: `tests/integration/test_run_command.py` runs `python -m acvp_runner run ...` as a subprocess against `aes-gcm-valid-encrypt` (PASS, stdout), `aes-gcm-valid-decrypt` (PASS, `--output`), and the new `aes-gcm-invalid-decrypt-tag` fixture (ERROR, non-zero exit); `tests/unit/test_runner.py` and `tests/unit/test_cli.py` cover the same paths in-process plus the missing-file and mismatched-identity cases.
...
- Project and task ID: ACVP Runner/Adapter - A14
...
- Project and task ID: ACVP Runner/Adapter - A15
...
- Project and task ID: ACVP Runner/Adapter - M01 (market roadmap)
...
- Commit/link/path: `scripts/fetch_vectors.py`, `scripts/dev.py`, `src/acvp_runner/{models,parser,runner}.py`, `fixtures/aes-gcm-decrypt-auth-failure/`, `tests/unit/test_fetch_vectors.py`, `tests/integration/test_nist_vectors.py`.
- Blocker, if any: none.
- Next unchecked ID: M02 - subprocess provider speaking JSON on stdin/stdout.

## 2026-09-02 - M02 external harness provider

- Project and task ID: ACVP Runner/Adapter - M02 (market roadmap)
...
- Evidence produced: `providers/subprocess_harness.py` implements the existing `AesGcmProvider` protocol over a subprocess, so nothing upstream of the provider boundary changed. `--provider-command` and `--provider-timeout` expose it on the CLI. `examples/reference_harness.py` is a complete worked harness that deliberately imports nothing from `acvp_runner`, which is what actually proves the A07 boundary is replaceable rather than merely typed. The full sixty-case pinned NIST set passes through the external process in about 3.8s, reported as provider `reference-harness`.
...
- Commit/link/path: `src/acvp_runner/providers/subprocess_harness.py`, `src/acvp_runner/cli.py`, `examples/reference_harness.py`, `tests/unit/test_subprocess_harness.py`, `tests/integration/test_run_command.py`.
- Blocker, if any: none.
- Next unchecked ID: M03 - SHA2 with Monte Carlo test support, then HMAC.

## 2026-09-02 - M03 SHA-2 and HMAC

- Project and task ID: ACVP Runner/Adapter - M03 (market roadmap)
...
- Commit/link/path: `src/acvp_runner/algorithms/`, `src/acvp_runner/providers/digest.py`, `src/acvp_runner/models.py`, `scripts/fetch_vectors.py`, `fixtures/sha2-256-known-answers/`, `fixtures/hmac-sha2-256-known-answers/`, `tests/unit/test_algorithms.py`, `tests/unit/test_digest_providers.py`.
- Blocker, if any: none.
...
- Project and task ID: ACVP Runner/Adapter - M04 (market roadmap)
...
- Commit/link/path: `src/acvp_runner/algorithms/ecdsa.py`, `src/acvp_runner/providers/ecdsa.py`, `src/acvp_runner/models.py`, `fixtures/ecdsa-p256-sigver/`, `tests/unit/test_ecdsa.py`.
- Blocker, if any: none.
- Next unchecked ID: M05 - ML-KEM and ML-DSA.

## 2026-09-02 - M05 ML-KEM and ML-DSA

- Project and task ID: ACVP Runner/Adapter - M05 (market roadmap)
...
- Tests run and result: `scripts/dev.py verify` passed Ruff, strict mypy, 247 pytest tests with 100% branch coverage, and wheel/sdist builds.
- Commit/link/path: `src/acvp_runner/algorithms/pqc.py`, `src/acvp_runner/providers/pqc.py`, `src/acvp_runner/providers/subprocess_harness.py`, `tests/unit/test_pqc.py`.
...
- Project and task ID: ACVP Runner/Adapter - M05 (follow-up)
...
- Commit/link/path: `examples/pqc_reference_harness.py`, `src/acvp_runner/algorithms/pqc.py`, `src/acvp_runner/providers/pqc.py`, `tests/unit/test_pqc.py`.
...
- Project and task ID: ACVP Runner/Adapter - M06 (market roadmap)
- Done condition: two reports from `acvp-runner run` can be compared, regressions are reported with the identifiers and diagnostics needed to act on them, and the exit code makes the comparison usable as a CI gate.
- Evidence produced: `src/acvp_runner/diff.py` plus an `acvp-runner diff BASELINE CURRENT [--output FILE]` subcommand. Changes are classified as regressed, coverage lost, fixed, still failing, or added; the verdict is REGRESSED, IMPROVED, or UNCHANGED; exit codes follow the existing scheme (0 nothing worse, 1 regression, 2 unreadable input).
...
- Commit/link/path: `src/acvp_runner/diff.py`, `src/acvp_runner/cli.py`, `tests/unit/test_diff.py`.
- Blocker, if any: none.

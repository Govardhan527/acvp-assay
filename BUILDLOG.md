...
- **The overrides were in the prose, not the table.** `docs/algorithm-frequency.md` ranks names by how many active FIPS 140-3 certificates carry them, and three entries have a caveat that contradicts the rank. Three-key TDES was marked `dead` in the table; DSA's decay under SP 800-131A, and the CNSA 2.0 date of January 2027 for ML-KEM and ML-DSA, were in paragraphs only. The file's own words were that they "deserve a caveat rather than a rank", which is the problem noticed and filed in the weakest available place: a consumer of the table, including the step that generates capability files from it, reads the column and never the paragraph.
- There is now a `trend` column from a closed set, `stable`, `decaying`, `mandated` and `dead`, and its source is `scripts/cavp_frequency.py`, which prints it. Each entry names the standard that determines it, and the TDES entries are read from the same `DEAD` set the unlock arithmetic uses, so there is one list rather than two. A test checks every row of the published table against the script, and the script's own report, run against the cache, agrees on all 88 names. The prose paragraphs stay: they carry the reasoning and the citations.
- A control built by counting a population measures what has been and cannot see a mandate. The table still ranks ML-KEM and ML-DSA near the bottom, confidently and with a real denominator; it now says in the data why that rank is not the build order.
- Populated from the existing prose and nothing more, which leaves one claim weaker than it looks. Every other name is marked `stable`, meaning no known policy pressure, and that has not been checked name by name: SHA-1, which NIST has announced it will retire, is the obvious one to examine. Adding it is a decision about a new citation, not a transcription of the ones already here.
...
- **An external harness could not say why it declined.** A harness answered `{"error": "unsupported"}` and every such case was filed as `implementation_lacks`, because the judgement lived in the algorithm modules and was based on what the built-in provider can do. With someone else's harness that judgement is in the wrong place: the harness knows whether a gap is its own or the vector's, and the runner cannot.
- A harness may now send `declineReason` and `detail` with a decline. It may claim only `implementation_lacks` or `vector_incomplete`. `runner_lacks` and `offline_undecidable` are properties of this runner and of its method, so a claim of either, or of any other value, is a protocol error that names the code. A decline without a reason still claims `implementation_lacks`, so an existing harness keeps its meaning. `detail` is at most 200 characters, because it is copied into the report.
- **Every UNSUPPORTED result names who declined it.** `DeclineClaimant` is `harness` or `runner`, and `TestCaseResult` refuses UNSUPPORTED without one, and refuses a harness claiming either of the runner's two reasons. The summary splits declines under `unsupportedByClaimant` and each case carries `declinedBy`, so a module author can read what their harness said about itself apart from what the runner decided.
- The 20 harness handlers across the algorithm modules were rewritten by a script to pass the harness's own claim through `declined_result`. It listed the two it could not place, the RSA group decline and the HMAC module's inline result, rather than guessing, and those were done by hand.
- **The script introduced a defect the tests missed and strict mypy caught.** In `aes_modes.py` it named the handler's variable `declined`, which that function already used for its kwCipher sentence. Python unbinds an `except ... as` name when the handler ends, so after a harness declined one KW case the next case raised `UnboundLocalError` instead of running. No test declined a KW case through a harness, which is why all 396 affected tests passed. The regression test was written first and watched failing with exactly that error before the variable was renamed.
- Tests run and result: `scripts/dev.py test` - format, lint, strict mypy, 1,039 passed and 9 skipped.
- Commit/link/path: `src/acvp_assay/models.py`, `src/acvp_assay/providers/subprocess_harness.py`, `src/acvp_assay/reporter.py`, `src/acvp_assay/runner.py`, `src/acvp_assay/algorithms/`, `docs/harness-protocol.md`, `README.md`.
- Blocker, if any: none.
- Next unchecked ID: a harness identifies itself by a name and a version, which two builds a commit apart share.
- **A harness was identified by a name and a version, which two builds a commit apart share.** That is the defect 0.22.0 fixed for this runner's own version, and it had not been required of anyone else's. The harness `metadata` answer now carries `buildId`, or `null` with `buildIdAbsentReason`: `not_exposed` when the implementation has a build identity its interface does not expose, and `not_recorded` when it records none. A harness that does not send the field at all still runs, and the report records `not_reported` with a note on stderr, because silently breaking every existing harness over a new field is exactly the cost the protocol should not impose. `not_reported` is this runner's to record, and a harness claiming it is a protocol error.
- `ProviderMetadata` enforces the pairing: an external provider carries a build id or the reason it has none, never both and never neither, and a built-in provider carries neither, since this runner's commit identifies its code. The report's provider block and `info --provider-command` carry both fields.
- **Checked on a real module rather than asserted.** Built against a scratch SoftHSM 2.6.1 token, the PKCS#11 example showed what `C_GetInfo` and `C_GetTokenInfo` actually supply: a manufacturer, a model, a label, a per-token serial number, and versions of two parts, so SoftHSM 2.6.1 reports itself as 2.6. Those calls name the module for free and cannot name a build, so `examples/pkcs11` now sends `not_exposed`, and `docs/harness-protocol.md` says so rather than implying the calls already carry one. The same run confirmed the first change on something other than the reference harness: `info` reported the harness as external, without the `cryptography` versions, and recorded `--pin 1234` as `--pin REDACTED`.
- The two Python reference harnesses send the SHA-256 of their own source as `buildId`, because a one-file harness is built from exactly that.
- Still open: a built-in run is identified by `runner_commit`, which `info` reports and a run report still does not carry.
- Tests run and result: `scripts/dev.py test` - format, lint, strict mypy, 1,052 passed and 9 skipped.
...
- **`not_exposed` described the right case and named a repair that cannot happen.** Its text said the vendor exposes the build identity. `CK_INFO` carries two bytes, a major and a minor shared by every build of a release, so no amount of vendor effort makes PKCS#11 carry a build: the reason code sent its reader somewhere that does not exist. Afchine Madjlessi reported it, with FreeHSM as the counterexample.
- **The assumption underneath was that a harness can only report what the interface tells it, and that is false.** A harness runs on the same host, holds the module path, and anything it can establish by honest means is a legitimate answer to `metadata`. FreeHSM has a build identity, a 48-byte digest in a dedicated ELF section that the module checks at load, and its 63 exported symbols are all `C_*`, so PKCS#11 cannot reach it and the harness can. This is the same mistake as `runtime_metadata()` hardcoding OpenSSL: a tool describing itself from the inside rather than from what is observable. Afchine has now found that assumption in two places without having written any of this code.
- `NOT_EXPOSED` now names the harness as the actor and says PKCS#11 will never carry it, so nobody waits for a vendor change that cannot come. `NOT_RECORDED` had a milder form of the same defect, since a harness can usually hash the artefact whatever the vendor records, and now says so: it fits mainly where there is no artefact to hash, such as a module reached over a wire.
- `docs/harness-protocol.md` gained a subsection on where a `buildId` may come from, with FreeHSM as the worked example, credited. It also says what a harness must not do: invent an identity, or send a version in place of one, since a version shared by every build of a release is the thing `buildId` exists to distinguish.
- No behaviour changed. A reason code's meaning is the same kind of change as code here, because one that names the wrong repair sends its reader nowhere.
- Tests run and result: `scripts/dev.py test` - format, lint, strict mypy, 1,052 passed and 9 skipped; documentation and a reason code's meaning, so the count is unchanged.
...
- **A run report named the implementation that answered and not the instrument that ran it.** 0.22.0 gave `info` a full `runner_commit` with a closed absent-reason set, and left reports carrying a version alone, so two result sets a commit apart were indistinguishable: the case where this matters is a regression hunt, which is exactly when a version is the one field that has not changed.
- Reports now carry a `runner` block with the same four fields as `info` and the same absent-reason set: `version`, `commit`, `commitAbsentReason`, `treeClean`. `build_report` and `report_json` take a keyword-only `runner`, measured from this checkout when it is not supplied, and the tests pass a fixed mapping so a report assertion does not depend on the checkout it runs in.
- `treeClean` is the field that keeps the block honest. A commit identifies the code only if the tree matched it, and during this very change a real run reported `treeClean: false`, because the working tree carried the edit that added the block.
- Checked on a real run and not only in the tests: a built-in AES-GCM run reported the full commit, a null `commitAbsentReason` and the version, and `acvp-assay diff` of two such reports still reports no differences, so the new top-level key does not disturb the comparison path.
- Tests run and result: `scripts/dev.py test` - format, lint, strict mypy, 1,054 passed and 9 skipped, coverage 94.82%.
- Commit/link/path: `src/acvp_assay/metadata.py`, `src/acvp_assay/reporter.py`, `tests/unit/test_reporter.py`, `tests/unit/test_cli.py`, `README.md`.
- **`info` named one library, and there are two.** Without a harness it reported `OpenSSL (via cryptography)`. The digests, HMAC, PBKDF, the protocol KDFs and both hash DRBGs answer through `hashlib`, which uses the interpreter's OpenSSL, while `cryptography` carries its own statically linked build: 3.5.8 and 4.0.2 respectively on this machine, both dated 25 Aug 2026. A single `openssl_version` key could not be right about both, so it was silently wrong about whichever family the reader had in mind.
- `provider` is now `built-in (cryptography and hashlib)` and `provider_libraries` lists each library with its own version and its own backend build. The two ambiguous keys are gone rather than kept beside the list, because a reader who finds `openssl_version` will use it. This changes the `info` payload, so it is a breaking change for anything parsing those keys.
- **The same wrong claim was one layer deeper, in run reports.** `providers/hash_drbg.py` declared `library_name="cryptography"` and reported that library's OpenSSL, while computing every digest with `hashlib` and every update with `hmac`, touching no `cryptography` primitive at all. Every HMAC-DRBG and Hash-DRBG run report therefore named a library that answered nothing and a build that did no work. Both now declare `hashlib` with the interpreter's OpenSSL, which `digest`, `pbkdf` and `kdf_tls` already did correctly. CTR-DRBG genuinely does use `cryptography`, through `Cipher` and `algorithms.AES`, and is untouched.
- Found and left, with the reason: `providers/kdf.py` is genuinely mixed, taking HMAC from `hmac` and CMAC-AES from `cryptography`, and declares only the latter. `ProviderMetadata` carries one library name, and which one answers is decided per test group rather than per provider, so reporting it truthfully is a design change and not a relabel. It is recorded here rather than half-fixed.
- Checked on live output, not only in tests: `info` prints both libraries with the two different OpenSSL builds, and the two hash DRBGs now report `hashlib 3.12.14` with the interpreter's OpenSSL. The vacuous half of two tests was fixed too: they asserted that keys absent for an external harness were absent, which a rename would have left trivially true, so they now assert the absence of `provider_libraries`.
- Tests run and result: `scripts/dev.py test` - format, lint, strict mypy, 1,054 passed and 9 skipped, coverage 94.82%.
- Commit/link/path: `src/acvp_assay/metadata.py`, `src/acvp_assay/providers/hash_drbg.py`, `tests/unit/test_metadata.py`, `tests/unit/test_cli.py`, `tests/integration/test_info_command.py`, `README.md`, `docs/limitations.md`, `docs/design.md`.
- **A descriptor could not survive `--provider-command`, so the safest way to pass a PIN was the one that did not work.** `examples/pkcs11` reads a PIN with `--pin-fd N`, and Python's `subprocess` closes every descriptor above 2, so the harness got `Bad file descriptor` and the only route was `sh -c 'exec ./acvp_harness ... --pin-fd 3 3<FILE'`, which asks the operator to write a shell incantation to use the option the tool recommends.
- `--provider-pass-fd N` names a descriptor the harness inherits. It is repeatable, threaded as a parameter rather than held in module state, because a process-global would leak between tests in one pytest run. It reaches `HarnessClient`, the eleven uniform `provider_for` factories, the DRBG factory, the three constructors whose `**kwargs: float` could not express a sequence of descriptors, all 21 dispatch functions, and the responder's `Harness`, which `scripts/acvts_client.py submit` drives: a flag that worked for `run` and silently did nothing for a live session would be worse than no flag.
- Two descriptors are refused, and both refusals are the point rather than politeness. Anything below 3 is rejected because 0, 1 and 2 carry the protocol, and a harness handed descriptor 0 would read its PIN from the request stream. A descriptor that is not open is rejected before any case runs, because otherwise it surfaces inside the vendor's process partway through a measurement as an unexplained failure.
- **Verified against a SoftHSM 2.6.1 token, in both directions.** With `--provider-pass-fd` the harness logged in and identified itself as SoftHSM 2.6 over PKCS#11 with no shell wrapper; without it, the same command failed with `cannot read the PIN from --pin-fd: Bad file descriptor` and the run stopped at exit 2, which is the defect being closed. While the harness was alive `/proc/PID/cmdline` carried `--pin-fd 3` and no PIN. A disposable token was initialised for this rather than reusing one whose PIN was not recorded.
- The `sh -c` form still works and stays in the documentation, because it is the only route on a runner older than this one. The protocol table now says a descriptor is inherited only with `--provider-pass-fd`, which is what a harness author needs to know before writing one.
- Tests run and result: `scripts/dev.py test` - format, lint, strict mypy, 1,061 passed and 9 skipped, coverage 94.84%.
- Commit/link/path: `src/acvp_assay/providers/subprocess_harness.py`, `src/acvp_assay/algorithms/`, `src/acvp_assay/responder.py`, `src/acvp_assay/cli.py`, `docs/harness-protocol.md`, `examples/pkcs11/README.md`.
- Blocker, if any: none.
- Next unchecked ID: `providers/kdf.py` reports one library for work split between `hmac` and `cryptography`; reporting it truthfully needs a per-mode provider identity rather than a rename.
